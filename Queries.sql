...
SELECT 
   city_name,
	...
	ROUND (SUM(s.total)/COUNT(DISTINCT c.customer_id),1) AS avg_sale_per_cx
FROM city AS ci
left join customers as c
on ci.city_id= c.city_id
JOIN sales AS s
...
SELECT 
    avg_sale_table.city_name,
    city.estimated_rent,
    avg_sale_table.total_customers,
    avg_sale_table.avg_sale_per_cx,
    ROUND( (city.estimated_rent/ avg_sale_table.total_customers),2) AS avg_rent_per_cx
FROM city
JOIN avg_sale_table
ON city.city_name=avg_sale_table.city_name
ORDER BY 5 DESC;
